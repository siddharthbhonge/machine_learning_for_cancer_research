			...
			for filename in os.listdir(idpath):
				...
				if filename.find("-") != -1:
					...
					if df.loc[df['filename'] == filename].md5.values[0] != filehash:
						logger.info("file id {} download fails, please download again".format(idname))
					else:
						count +=1
	if count == total:
...
if __name__ == '__main__':
	...
	manifest_file = "/home/siddharth/Downloads/gdc_manifest.2018-10-19.txt"
	df = pd.read_csv(manifest_file,sep='\t')
	total = df.shape[0]
	print("Total:"+str(total))
	
	# The directory that holds the data. Modify this when use.
	dirname = "/home/siddharth/Downloads"
	check(dirname,total)
